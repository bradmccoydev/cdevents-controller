...
import (
	...
	"github.com/bradmccoydev/cdevents-controller/pkg/api"
	"github.com/bradmccoydev/cdevents-controller/pkg/grpc"
	"github.com/bradmccoydev/cdevents-controller/pkg/signals"
...
func main() {
	...
	fs.String("grpc-service-name", "cdevents-controller", "gPRC service name")
	...
	fs.String("ui-message", fmt.Sprintf("greetings from cdevents-controller v%v", version.VERSION), "UI message")
	...
	viper.SetEnvPrefix("cdevents-controller")
	...
	logger.Info("Starting cdevents-controller",
...
func beginStressTest(cpus int, mem int, logger *zap.Logger) {
	...
	if mem > 0 {
		path := "/tmp/cdevents-controller.data"
